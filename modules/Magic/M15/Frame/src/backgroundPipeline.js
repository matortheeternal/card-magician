...
export const buildPipeline = (utils) => [{
    ...
    apply: async (card, bgs) => {
        ...
        bgs.base = await background(utils, `${card.frameFolder}/${key}.jpg`, 0);
    }
}, {
    name: 'hybrid',
    useBackground: card => card.colorIdentity.isHybrid(),
    apply: async (card, bgs) => {
        let [c1, c2] = card.colorIdentity.colors.map(c => c.char);
        if (card.isLand()) c1 += 'l', c2 += 'l';
        const images = await Promise.all([
            utils.assetURL(`${card.frameFolder}/${c1}.jpg`),
            utils.assetURL(`${card.frameFolder}/${c2}.jpg`)
        ]);
    ...
    apply: async (card, bgs) => {
        ...
        const artifactUrl = await utils.assetURL(`${card.frameFolder}/${key}.jpg`);
    ...
    useBackground: card => card.isEnchantment() &&
        card.colorIdentity.isHybrid() && !card.isSnow(),
    apply: async (card, bgs) => {
        const [c1, c2] = card.colorIdentity.colors.map(c => c.char);
        const images = await Promise.all([
            utils.assetURL(`nyx/${c1}.jpg`),
            utils.assetURL(`nyx/${c2}.jpg`)
        ]);
        const url = await utils.linearBlend(...images, 0.4, 0, 0.6, 0);
    ...
    apply: async (card, bgs) => {
        const [c1, c2] = card.colorIdentity.colors.map(c => c.char);
        const images = await Promise.all([
            utils.assetURL(resolveCrown(card, c1)),
            utils.assetURL(resolveCrown(card, c2))
        ]);
        const url = await utils.linearBlend(...images, 0.4, 0, 0.6, 0);
